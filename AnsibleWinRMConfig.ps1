...
if($EnableHTTP){
    If (!(Get-PSSessionConfiguration -Verbose:$false) -or (!(Get-ChildItem WSMan:\localhost\Listener | Where -Property Keys -eq "Transport=HTTP")))
    {
...
# If DisableHTTP if set to true
If ($DisableHTTP)
{
    if(Get-ChildItem WSMan:\Localhost\Listener | Where -Property Keys -eq "Transport=HTTP"){
        Write-Verbose "Disabled WinRM over HTTP."
        ...
        Write-Verbose "WinRM over HTTP is already disabled"
    }
}
...
$httpResult = Get-ChildItem WSMan:\Localhost\Listener | Where -Property Keys -eq "Transport=HTTP"
$httpsResult = Get-ChildItem WSMan:\Localhost\Listener | Where -Property Keys -eq "Transport=HTTPS"
